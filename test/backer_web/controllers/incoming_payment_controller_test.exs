defmodule BackerWeb.IncomingPaymentControllerTest do
  ...
  @create_attrs %{action: "some action", amount: 42, destination: "some destination", details: "some details", evidence: "some evidence", source: "some source", status: "some status", tx_id: "some tx_id"}
  ...
  describe "edit incoming_payment" do
    ...
    test "renders form for editing chosen incoming_payment", %{conn: conn, incoming_payment: incoming_payment} do
  ...
  describe "update incoming_payment" do
    ...
    test "redirects when data is valid", %{conn: conn, incoming_payment: incoming_payment} do
      conn = put conn, incoming_payment_path(conn, :update, incoming_payment), incoming_payment: @update_attrs
    ...
    test "renders errors when data is invalid", %{conn: conn, incoming_payment: incoming_payment} do
      conn = put conn, incoming_payment_path(conn, :update, incoming_payment), incoming_payment: @invalid_attrs
      assert html_response(conn, 200) =~ "Edit Incoming payment"
    end
  end
