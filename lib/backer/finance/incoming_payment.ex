  ...
  defp validate_entry(changeset) do
    ...
    return =
      if status != "Approved" do
        ...
        case action do
          "Settle Invoice" ->
            changeset |> put_change(:backer_id, nil) |> validate_required([:invoice_id])

          "Deposit" ->
            changeset |> put_change(:invoice_id, nil) |> validate_required([:backer_id])

          "Other" ->
            changeset |> put_change(:backer_id, nil) |> put_change(:invoice_id, nil)

          _ ->
            add_error(
              changeset,
              :status,
              "Something is wrong. Tell developer there's constant inconsistency"
            )
        end
      end
  end
    ...
    return =
      if status != "Executed" do
        ...
        add_error(
          changeset,
          :status,
          "This incoming payment cannot be edited because already executed"
        )
      end
  end
