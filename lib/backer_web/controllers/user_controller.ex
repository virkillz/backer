defmodule BackerWeb.UserController do
  ...
  case Account.authenticate_user(user, password) do
    {:ok, user} ->
      ...
      |> render("login.html", layout: {BackerWeb.LayoutView, "center.html"}, changeset: changeset)        
        else
  ...
  def locked(conn, _) do
    ...
    conn
    |> Guardian.logout
    |> render("locked.html", layout: {BackerWeb.LayoutView, "center.html"}, name: name, avatar: avatar, username: username, changeset: changeset)
  end
  ...
  def profile(conn, _) do
    ...
    history = histories |> Enum.map(fn x -> Map.from_struct(x) end) |> Enum.map(fn x -> getday(x) end) |> Enum.map(fn x -> getago(x) end) |> Enum.group_by(fn x -> x.day end) |> IO.inspect
    render(conn, "profile.html", logs: histories, user: user, changeset: changeset, history: history)
  end

  defp getago(activity) do
    {:ok, relative_str} = Timex.shift(activity.inserted_at, minutes: -3) |> Timex.format("{relative}", :relative)
